...
export default function ProjectCard({
      ...
      <div className="flex flex-wrap gap-2 mt-auto relative z-20">
        {tags.map((tag) => (
          <span
            key={tag}
            className={cn(
              "px-3 py-1 text-xs font-medium glass rounded-full",
              tag.toLowerCase() === "planned" ? "text-amber-400/80 bg-amber-500/5" : "text-gray-300"
            )}
          >
            {tag}
          </span>
  ...
  return (
    <motion.div
      whileHover={isPlanned ? {} : { y: -5 }}
      className={cn(
        "group relative block h-full overflow-hidden rounded-2xl p-px transition-all duration-300",
        isPlanned 
          ? "bg-white/5 opacity-70" 
          : featured 
            ? "bg-gradient-to-br from-[#00F0FF] to-[#8A2BE2]" 
            : "bg-white/10 hover:bg-white/20"
      )}
    >
      {isPlanned ? (
        <div className="relative z-10 flex h-full flex-col justify-between bg-[#0A0A0A] p-8 rounded-[15px] transition-colors">
          {content}
        </div>
      ) : (
